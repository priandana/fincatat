        ...
        <div className="w-full max-w-md bg-[#f2f6fa] relative h-full shadow-2xl flex flex-col overflow-hidden">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {appData?.categories[activeScreen === 'form_in' ? 'in' : 'out'].map(cat => (
                        <button key={cat} onClick={() => setFormData({...formData, category: cat})} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${formData.category === cat ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>{cat}</button>
                      ))}
                    </div>
                  </div>
            ...
            {activeScreen === 'history' && (
              <div className="animate-fade-in-up">
                ...
                <div className="bg-white -mx-5 px-6 py-8 rounded-t-[45px] min-h-[600px] shadow-inner space-y-4">
                  ...
                  {(!appData?.transactions || appData?.transactions?.length === 0) && <div className="text-center py-20 text-slate-300 font-bold">Belum ada transaksi</div>}
                </div>
              </div>
            )}
          ...
          <div className="w-full h-20 bg-white/90 backdrop-blur-md rounded-t-[30px] shadow-[0_-10px_25px_rgba(0,0,0,0.05)] flex justify-between items-end px-4 pb-2 z-50 border-t border-white shrink-0">
            {[
              ...
              <div key={nav.id} onClick={() => setActiveScreen(nav.id)} className={`flex flex-col items-center justify-center cursor-pointer transition-all duration-500 ${activeScreen === nav.id ? 'w-[75px] h-[85px] bg-[#1d2333] text-white rounded-t-[28px] shadow-2xl -translate-y-2' : 'w-16 h-16 text-slate-400 mb-2 hover:text-[#20b2aa]'}`}>
